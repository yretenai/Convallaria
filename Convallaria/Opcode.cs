...
public enum Opcode : byte {
	...
	NewTable,	// A B C k  R[A] := {}, k = use extra
	...
	MmBinI,		// A sB C k call C metamethod over R[A] and sB, k = flip order
	MmBinK,		// A B C k  call C metamethod over R[A] and K[B], k = flip order
	Unm,		// A B      R[A] := -R[B]
	BNot,		// A B      R[A] := ~R[B]
	Not,		// A B      R[A] := not R[B]
	...
	TailCall,	// A B C k  return R[A](R[A+1], ... ,R[A+B-1]), k = close upvals
	Return,		// A B C k  return R[A], ... ,R[A+B-2]      (see note), k = close upvals
	Return0,	//          Return
	...
	SetList,	// A B C k  R[A][C+i] := R[A+i], 1 <= i <= B, k = use extra
